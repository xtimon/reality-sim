...
class QuantumFabric:
    ...
    def __init__(self, num_qubits: int, entanglement_strength: float = 1.0):
        """
    ...
    def apply_entanglement_operator(self, qubit_pairs: List[Tuple[int, int]], 
                                    use_hadamard: bool = True) -> np.ndarray:
        """
    ...
    def _create_entanglement_operator(self, control: int, target: int) -> np.ndarray:
        """
            ...
            for i in range(size):
                ...
                if control_bit == 1:
                    ...
                    if target_bit == 0:
                        ...
                        operator[i, i] = np.cos(theta / 2)
                        operator[target_state, i] = -np.sin(theta / 2)
                    else:
                        # Исходное состояние |1,1>
                        operator[i, i] = np.cos(theta / 2)
                        source_state = i ^ (1 << target)  # Состояние с флипнутым target
                        operator[source_state, i] = np.sin(theta / 2)
                else:
                    # Если control = 0, ничего не меняем
                    operator[i, i] = 1.0
    ...
    def apply_hadamard(self, qubit_index: int) -> np.ndarray:
        """
    ...
    def _is_unitary(self, operator: np.ndarray, tolerance: float = 1e-10) -> bool:
        ...
        return np.allclose(operator @ operator.conj().T, np.eye(len(operator)), atol=tolerance)
    ...
    def get_state_info(self) -> str:
        ...
        """
        norm = np.linalg.norm(self.state)
        coherence = np.sum(np.abs(self.state)**2)
    ...
    def get_entanglement_entropy(self) -> float:
        ...
        if self.n == 2:
            ...
            eigenvals = np.real(eigenvals)  # Убираем мнимую часть (должна быть 0)
            ...
            for i in range(self.n):
                for j in range(i + 1, self.n):
                    ...
                    # Вычисляем энтропию
                    eigenvals = np.linalg.eigvals(rho_i)
                    eigenvals = np.real(eigenvals)
                    eigenvals = eigenvals[eigenvals > 1e-10]
                    
                    if len(eigenvals) > 0 and np.sum(eigenvals) > 1e-10:
